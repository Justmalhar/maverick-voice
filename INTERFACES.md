...
**Notes:** Port the activation-mode + chaining + debounce state machine VERBATIM (platform-agnostic). Constants `DEBOUNCE_MS=300`, `chainWindowMs=2000`, `DUAL_HOLD_MS=400`, `DUAL_DOUBLE_TAP_MS=400`. Debounce is per-logical-key, gates START only (never STOP). `resetState()` MUST clear EVERY field influencing the next keystroke (including instruction-key routing state) — but NOT the persisted `instructionEnabled` flag (see Feature delta below; it is restored at boot + via live IPC). `setActivationMode` resets dual state.

**Key vocabulary (CRITICAL — platform-invariant):** `handleKey` consumes `KeyEvent` strings (dashed-lowercase) from `keyListener`. The DICTATION key (whichever the platform/config maps) arrives as `'dictation-down'`/`'dictation-up'` — i.e. **keyListener normalizes the physical dictation key to a logical `dictation-down/up`**, so keyboard.ts is platform-unaware for dictation. INSTRUCTION arrives as `'instruction-down'` (the instruction key is **Caps Lock on both platforms**; `InstructionKey` is the single-member union `'caps-lock'`). Handle instruction toggle on **`instruction-down` only** — `'instruction-up'` is **never emitted** on either platform (darwin collapses the Caps LED pair; win32 suppresses the up for parity) and is an explicit no-op. **Instruction mode is opt-in (default OFF):** `keyboardManager.setInstructionEnabled(false)` makes `handleKey` ignore ALL instruction-derived events (and the dictation→instruction chain transition); `resetState()` must NOT clear that persisted flag. `[keyboard]` log prefix. STRIP `authToken` references seen in reference logging.
...
- **darwin:** spawn `resources/bin/globe-listener` (binary already in repo). Parse its stdout protocol (verbatim below) and TRANSLATE raw tokens → normalized KeyEvents based on the configured `dictationKey`/`instructionKey`:
  ...
  - `CAPS_DOWN`+`CAPS_UP` → collapse to a SINGLE `instruction-down` per physical press (instructionKey is always `'caps-lock'`). LED toggle: both tokens fire for one press; a `capsExpectingSecondToken` flag emits `instruction-down` on the FIRST token and swallows the second. No `instruction-up` is synthesized — keyboard.ts triggers on down only.
  - **Right Shift is GONE.** It was removed entirely (system-shortcut + Shift+Enter conflicts). The instruction key is **Caps Lock on both platforms**; `InstructionKey` is the single-member union `'caps-lock'`. The shipped globe-listener never emitted a right-shift token anyway. Do not re-add it; `main.ts restoreSettings()` migrates a persisted `'right-shift'` to `'caps-lock'` at boot.
...
- **win32:** `import { uIOhook, UiohookKey } from 'uiohook-napi'`. On `start()`: register `uIOhook.on('keydown', e => ...)` and `'keyup'`, then `uIOhook.start()`. Map `e.keycode` against `UiohookKey` **by NAME** (do NOT use raw numbers):
  ...
  | instruction (only) | `'caps-lock'` | `UiohookKey.CapsLock` |
  Emit `dictation-down`/`dictation-up`/`instruction-down` accordingly. **Suppress typematic auto-repeat:** track a physical-down flag per key (`win32DictationDown`/`win32InstructionDown`) and emit exactly ONE `*-down` per press (Windows fires keydown repeatedly while held, past the 300ms debounce). **Do NOT emit `instruction-up` on win32** — parity with darwin's LED-pair collapse (which produces only `instruction-down`). `stop()` calls `uIOhook.stop()` and clears the down flags. `sendCommand` rejects/no-ops on win32. `getBinaryPath()` returns null on win32 (harmless).
...
**Notes:** Port near-verbatim. Main window: 900x640, `minWidth:700`, `minHeight:500`, contextIsolation, preload `../preload/preload.js`, loads `index.html`. HUD: 520x140 frameless transparent always-on-top, loads `index.html#/widget`. **CHANGE `backgroundColor` to pure black (`#000000`).** KEEP the readiness handshake: `hudReadyPromise` resolved by `markHUDReady()`, awaited by `showHUD()`; `resetHUDReady()` rebound on hudWindow `did-start-loading`. KEEP `showHUD`'s **double `cancelPendingHide()`** (before AND after the await). KEEP `hideHUD`'s **220ms** setTimeout (matches `.animate-hud-exit` 200ms + buffer). KEEP `showInactive()` + `setAlwaysOnTop('floating')` + `moveTop()` ordering. `getHUDBounds` uses `screen.getPrimaryDisplay().workArea` and **bottom-anchors** the pill: `y = workArea.y + workArea.height - HUD_HEIGHT - DOCK_CLEARANCE` (`HUD_HEIGHT=140`, `DOCK_CLEARANCE=80`; `workArea` already excludes the Dock at any size/side, even auto-hidden). `x` is centered or right-aligned (12px right inset). The pill bottom-anchors inside the 520×140 canvas (area above is transparent + click-through). `display-metrics-changed` reposition listener kept. Do not move it back to the top of the screen.
...
**Pipeline (SEQUENTIAL ONLY — strip the fast path):** transcribe via `getTranscriptionProvider(stt.provider).transcribe(buffer, {model:stt.model, language: stt.language==='auto'?undefined:stt.language, mimeType:'audio/webm'}, getApiKey(stt.provider)!, signal)`. Pure dictation → `cleanTranscript` only, NEVER hits LLM **by default** (the opt-in AI auto-format pass is the one exception — see Feature delta below; pipeline order becomes clean → Dictionary → Snippets → optional auto-format). transform/context/instruction → `assembleTransformMessages(...)` then `getLLMProvider(llm.provider).complete({model:llm.model, system, user, temperature, maxTokens:APP_CONFIG... (use 4096), baseUrl:llm.baseUrl||undefined, timeoutMs:APP_CONFIG.transform.timeout_ms}, getApiKey(llm.provider)!, signal)`. On LLM empty/refusal/error → fall back to raw transcript, `output:fallback` with `formattingNotice()`. `quote` flow = `'> '+selectedText`, no LLM. Key presence: check `hasApiKey(provider)` before calling; surface `NoApiKeyError` via `simplifyError`.
...
**Settings persistence:** electron-store. Keys: `widgetPosition`('center'), `soundFeedback`(true), `chunkedTranscription`(true), `outputMode`('paste'), `inputDeviceId`(''), `dictationKey`(platform default: darwin `'fn'`, win32 `'right-ctrl'`), `instructionKey`('caps-lock' on both platforms; a persisted `'right-shift'` is migrated to `'caps-lock'` at boot), `activationMode`('tap-toggle'), `sttSettings`({provider:'groq',model:'whisper-large-v3-turbo',language:'en'}), `llmSettings`({provider:'openai',model:'gpt-4o-mini',baseUrl:''}), plus the Feature delta keys `instructionEnabled`(false), `autoFormat`(false), `dictionary`([]), `snippets`([]). On restore, push into sessionManager + keyboardManager + keyListener + windowManager.
...
Default export `App()`. View machine loading→onboarding→main, gated by `localStorage['maverickvoice_onboarding_complete']`. Sidebar tabs: **Home / History / Dictionary / Snippets / Settings** (`Tab = 'home'|'history'|'dictionary'|'snippets'|'settings'`). Usage stats + Privacy are folded into Settings; the Voice/"Features" explainer is a block inside Home. STRIP auto-update banner. Brand: "Maverick Voice". Pro-tip uses dynamic dictation key from `getDictationKey()`; instruction key is Caps Lock (opt-in, off by default).

## `renderer/app/Settings.tsx`
Default `Settings({ onDictationKeyChange? })`. Sections: Permissions (mic + accessibility live status w/ window-focus recheck; win32 auto-granted/hidden), **Provider keys** (per-provider cards for Groq STT / OpenAI / OpenRouter via `getProviderKeyStatus`/`setProviderKey`/`testProviderKey`/`clearProviderKey`; OpenAI+OpenRouter cards also edit model dropdown via `listModels` + baseUrl input via `getLLMSettings`/`setLLMSettings`; Groq card edits STT model + language via `getSTTSettings`/`setSTTSettings`), Usage (`getUsage`/`resetUsage`, per-provider cost), Keyboard Shortcuts (dictation key segmented control — platform-aware: darwin Fn/Right-Option, win32 Right-Ctrl/Right-Alt; instruction key is Caps Lock on both platforms; activation mode), Audio (enumerate mic devices), Behavior (output paste|clipboard if exposed, sound feedback, chunked transcription, **AI auto-format toggle** via `get/setAutoFormat`), **Advanced** (**instruction-mode opt-in** via `get/setInstructionEnabled` — default off), Appearance (widget position center|right), Privacy (folded in), Usage (`getUsage`/`resetUsage`, per-provider cost), Help (replay onboarding). STRIP all whisper/local-model UI + cloud-vs-local STT toggle + Hinglish. Use `.btn-glass`, `.kbd-3d`, glass cards.
...
- `UiohookKey.CapsLock` — win32 instruction key (only binding; Right Shift removed)
- `UiohookKey.Escape` — if needed for renderer-side; Escape cancel is handled in main via globalShortcut, so not required here.

Event handlers: `uIOhook.on('keydown', (e) => { if (e.keycode === UiohookKey.CapsLock) { if (win32InstructionDown) return; win32InstructionDown = true; emit('key','instruction-down') } ... })` and `'keyup'` (clears the down flag). Start once with `uIOhook.start()`, stop with `uIOhook.stop()`. Suppress typematic auto-repeat (one `*-down` per press) and **do not emit `instruction-up`** — parity with darwin's LED-pair collapse; keyboard.ts triggers on `instruction-down` only.
