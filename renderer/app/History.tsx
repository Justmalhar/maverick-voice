              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mb-2.5">
                    ...
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${flow.tier}`}>
                      {flow.label}
                    </span>
                    {isRetrying ? (
                      <span className="text-[10px] font-semibold text-mv-text-primary bg-mv-white-08 border border-mv-white-12 px-2 py-0.5 rounded-full">
                        Retrying…
                      </span>
                    ) : isError ? (
                      <span className="text-[10px] font-semibold text-mv-text-secondary bg-mv-white-04 border border-mv-white-12 px-2 py-0.5 rounded-full">
                        Failed
                      </span>
                    ) : null}
                  </div>

                  {/* Output / re-processing state */}
                  {isRetrying ? (
                ...
                {/* Static waveform decoration — hidden while retrying */}
                {!isRetrying && (
                  <div className="flex items-center gap-[2px] h-[28px] opacity-[0.1] shrink-0 mr-1">
                    {[6, 14, 10, 18, 8, 16, 12, 20].map((h, i) => (
                  ...
                  <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-200 shrink-0 translate-y-1 group-hover:translate-y-0">
                    {session.output && (
                      <button
                        ...
                        className={`w-9 h-9 rounded-mv-md flex items-center justify-center border transition-all duration-200 ${
                          isCopied
                    ...
                    {hasAudio && (
                      <button
                        ...
                        className="w-9 h-9 rounded-mv-md flex items-center justify-center border bg-mv-white-04 border-mv-border text-mv-text-muted hover:text-mv-text-primary hover:bg-mv-white-08 transition-all duration-200"
