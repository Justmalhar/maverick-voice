// ─── Shared system prompts + message assembly ───
// These prompts are tuned and ported VERBATIM from unmute-dictation. They encode
// a text-transformation engine that NEVER answers the content and NEVER wraps
// its output in quotation marks (the output is pasted directly at the cursor).
//
// Maverick Voice change: assembleTransformMessages returns { system, user,
...
/** The flows the transform engine assembles prompts for. */
...
/**
 * Assemble the { system, user, temperature } a single LLMProvider.complete call
 * needs. Replicates the reference assembleTransformMessages logic, folding the
 * reference's separate dictation user message into a single `user` string.
 *
 * The 'quote' flow has no LLM (sessionManager handles it as '> ' + selectedText),
