...
export function initDB(): void {
  const dbPath = path.join(app.getPath('userData'), 'maverickvoice.db')
  ...
  // Cumulative provider usage, one row per local day + model. Deliberately NOT
...
export function getSessions(limit = 50): Record<string, unknown>[] {
  ...
  // Map snake_case DB columns to camelCase for renderer (matches Session type)
  return rows.map((row) => ({
    id: row.id,
