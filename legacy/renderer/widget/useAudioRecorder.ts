...
// The widget records in exactly the two logical session modes.
type RecordingMode = SessionMode
...
// ─── VAD Chunking Defaults (overridden by AppConfig.chunking at recording start) ───
...
export function useAudioRecorder(): UseAudioRecorderReturn {
  ...
  // ─── Config-driven chunking params (loaded at recording start from AppConfig) ───
  ...
  const emitChunk = useCallback(async (reason: 'silence' | 'hard-cap'): Promise<void> => {
    ...
    console.log(
      `[audio:vad] ${reason === 'silence' ? 'Silence detected' : 'Hard cap'}, cutting chunk ${chunkIdx} at ${elapsed}ms (${buffer.byteLength} bytes)`
    )
  ...
  const flushRecorder = useCallback(async (): Promise<boolean> => {
    ...
    if (chunkedModeEnabledRef.current && chunkIndexRef.current > 0) {
      ...
      if (macroBlobs.length > 0) {
        ...
        console.log(
          `[audio] Flushed FINAL chunk ${chunkIndexRef.current}/${totalChunks}, size: ${buffer.byteLength}, duration: ${duration}ms`
        )
        window.electronAPI.sendAudioFinalChunk(
          buffer,
          chunkIndexRef.current,
          totalChunks,
          duration,
          mode,
          frozenSessionIdRef.current
        )
      } else {
  ...
  const startRecording = useCallback(
    async (deviceId?: string, mode?: RecordingMode, sessionId?: string) => {
      ...
      // Load config-driven chunking params (non-blocking — falls back to defaults)
      try {
        ...
        if (config?.chunking) {
          ...
          console.log(
            `[audio] Loaded chunking config v${config.version}: min=${chunkMinMsRef.current}ms, silence=${silenceThresholdRef.current}, hardCap=${hardChunkCapMsRef.current}ms`
          )
        }
      } catch {
        console.log('[audio] Could not load app config, using default chunking params')
      }
      ...
      recorder.onstop = async () => {
        ...
        if (chunkedModeEnabledRef.current && chunkIndexRef.current > 0) {
          ...
          if (macroBlobs.length > 0) {
            ...
            const buffer = await blob.arrayBuffer()
            console.log(
              `[audio] Sending FINAL chunk ${chunkIndexRef.current}/${totalChunks}, size: ${buffer.byteLength}, duration: ${duration}ms`
            )
            window.electronAPI.sendAudioFinalChunk(
              buffer,
              chunkIndexRef.current,
